...
func GetTenantId(p *graphql.ResolveParams) (uuid.UUID, map[string]interface{}) {
	// Extract the HTTP request from the context
	req, ok := p.Context.Value("httpRequest").(*http.Request)
	if !ok {
		return uuid.Nil, common.ResponseError("failed to get HTTP request from context")
	}

	// Get Tenant ID from the header
	tenantIDStr := req.Header.Get("X-Tenant-ID")
	if tenantIDStr == "" {
		return uuid.Nil, common.ResponseError("missing X-Tenant-ID header")
	}

	// Parse Tenant ID to UUID
	tenantID, err := uuid.Parse(tenantIDStr)
	if err != nil {
		return uuid.Nil, common.ResponseError("invalid Tenant ID format")
	}
